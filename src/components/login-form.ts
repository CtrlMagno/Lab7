...
export class LoginForm extends HTMLElement {
    ...
    private async handleSubmit(event: Event) {
        ...
        if (this.failedAttempts >= 3 && now - this.lastAttemptTime < 30000) {
            const remainingTime = Math.ceil((30000 - (now - this.lastAttemptTime)) / 1000);
            if (errorElement) {
                errorElement.textContent = `Too many failed attempts. Please wait ${remainingTime} seconds before trying again.`;
